...
class TraderWindow:
    ...
    def __init__(self, parent, player, trader, on_close_callback):
        ...
        self.window.title(get_text('trader_title'))
    ...
    def create_widgets(self):
        ...
        ttk.Label(top_frame, text=get_text('your_coins_label')).pack(side=tk.LEFT)
        ttk.Label(top_frame, textvariable=self.player_copper_var).pack(side=tk.LEFT, padx=5)

        self.upgrade_button = ttk.Button(top_frame, text=get_text('expand_inventory_button'), command=self.buy_upgrade)
        ...
        sell_frame = ttk.LabelFrame(paned_window, text=get_text('your_inventory_sell_title'), padding="10")
        ...
        buy_frame = ttk.LabelFrame(paned_window, text=get_text('trader_offer_buy_title'), padding="10")
        ...
        self.sell_button = ttk.Button(sell_buttons_frame, text=get_text('sell_button'), command=self.sell_item)
        self.sell_button.pack(fill=tk.X, expand=True, side=tk.LEFT, padx=(0, 5))
        self.sell_all_button = ttk.Button(sell_buttons_frame, text=get_text('sell_junk_button'), command=self.sell_all_non_upgrades)
        self.sell_all_button.pack(fill=tk.X, expand=True, side=tk.LEFT)

        self.buy_button = ttk.Button(bottom_frame, text=get_text('buy_button'), command=self.buy_item)
    ...
    def update_display(self):
        ...
        self.upgrade_cost_var.set(f"{get_text('cost_label')} {format_currency(self.trader.get_upgrade_cost())}")
    ...
    def sell_item(self):
        ...
        if not selected_indices:
            messagebox.showwarning(get_text('sell_warning_title'), get_text('sell_warning_message'), parent=self.window)
            return
    ...
    def sell_all_non_upgrades(self):
        ...
        if items_sold > 0:
            messagebox.showinfo(get_text('sell_all_info_title'),
                                get_text('sell_all_info_message').format(count=items_sold, amount=format_currency(copper_gained)),
                                parent=self.window)
            self.update_display()
        else:
            messagebox.showinfo(get_text('nothing_to_sell_title'), get_text('nothing_to_sell_message'), parent=self.window)

    def buy_upgrade(self):
        """Buys an inventory upgrade."""
        cost = self.trader.get_upgrade_cost()
        if self.trader.buy_inventory_upgrade(self.player):
            messagebox.showinfo(get_text('upgrade_success_title'), get_text('upgrade_success_message').format(cost=format_currency(cost)), parent=self.window)
            self.update_display()
        else:
            messagebox.showerror(get_text('not_enough_coins_title'), get_text('not_enough_coins_message'), parent=self.window)

    def buy_item(self):
        """Buys the selected item from the trader."""
        selected_indices = self.buy_listbox.curselection()
        if not selected_indices:
            messagebox.showwarning(get_text('buy_warning_title'), get_text('buy_warning_message'), parent=self.window)
            return
        item_to_buy = self.potions_for_sale[selected_indices[0]]
        success, message = self.trader.buy_item(self.player, item_to_buy)
        if success:
            self.update_display()
        else:
            messagebox.showerror(get_text('buy_fail_title'), get_text(message), parent=self.window)
